...
impl TcpStream {
    cfg_not_wasi! {
        ...
        pub async fn connect<A: ToSocketAddrs>(addr: A) -> io::Result<TcpStream> {
            // print
            println!("TcpStream::connect, before to_socket_addrs");
            let addrs = to_socket_addrs(addr).await?;
            println!("TcpStream::connect, after to_socket_addrs");
            ...
            for addr in addrs {
                match TcpStream::connect_addr(addr).await {
                    Ok(stream) => {
                        println!("TcpStream::connect, connect success");
                        return Ok(stream)
                    },
                    Err(e) => {
                        println!("TcpStream::connect, connect failed, error: {:?}", e);
                        last_err = Some(e)
                    },
                }
            }

            println!("TcpStream::connect, before return final error");
        ...
        async fn connect_addr(addr: SocketAddr) -> io::Result<TcpStream> {
            println!("TcpStream::connect_addr, before connect");
            let sys = mio::net::TcpStream::connect(addr)?;
            println!("TcpStream::connect_addr, after connect");
        ...
        pub(crate) async fn connect_mio(sys: mio::net::TcpStream) -> io::Result<TcpStream> {
            ...
            println!("TcpStream::connect_mio, before poll_write_ready");
            poll_fn(|cx| stream.io.registration().poll_write_ready(cx)).await?;
            println!("TcpStream::connect_mio, after poll_write_ready");

            if let Some(e) = stream.io.take_error()? {
                println!("TcpStream::connect_mio, take_error, error: {:?}", e);
                return Err(e);
            }
